...
function validateForm(formId) {
  ...
  form.querySelectorAll('[data-email]').forEach(el => {
    if (el.offsetParent === null || !el.value) return;
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(el.value)) {
      if (!el.classList.contains('input-error')) showError(el, 'Enter a valid email address');
      valid = false;
    }
  });
  form.querySelectorAll('[data-min], [min]').forEach(el => {
    if (el.offsetParent === null || !el.value) return;
    const minVal = el.dataset.min !== undefined ? el.dataset.min : el.getAttribute('min');
    if (Number(el.value) < Number(minVal)) {
      if (!el.classList.contains('input-error')) showError(el, `Minimum value is ${minVal}`);
      valid = false;
    }
  });
  form.querySelectorAll('[data-max], [max]').forEach(el => {
    if (el.offsetParent === null || !el.value) return;
    const maxVal = el.dataset.max !== undefined ? el.dataset.max : el.getAttribute('max');
    if (Number(el.value) > Number(maxVal)) {
      if (!el.classList.contains('input-error')) showError(el, `Maximum value is ${maxVal}`);
      valid = false;
    }
  });
  form.querySelectorAll('[data-maxlen]').forEach(el => {
    if (el.offsetParent === null || !el.value) return;
    if (el.value.length > Number(el.dataset.maxlen)) {
      if (!el.classList.contains('input-error')) showError(el, `Maximum ${el.dataset.maxlen} characters`);
      valid = false;
    }
  });

  form.querySelectorAll('input[type="number"][id*="progress"]').forEach(progEl => {
    if (progEl.offsetParent === null || progEl.value === '') return;
    const statEl = form.querySelector('select[id*="status"]');
    if (statEl && statEl.offsetParent !== null && statEl.value) {
      const p = Number(progEl.value);
      const s = statEl.value;
      if (s === 'Completed' && p !== 100) {
        if (!statEl.classList.contains('input-error')) showError(statEl, "Status cannot be 'Completed' unless progress is 100%");
        valid = false;
      } else if (p === 100 && s !== 'Completed') {
        if (!progEl.classList.contains('input-error')) showError(progEl, "Progress is 100%, status must be 'Completed'");
        valid = false;
      }
    }
  });
